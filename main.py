...
class LabelSmoothingLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        with torch.no_grad():
            true_dist = torch.zeros_like(pred) # shape : bz,NClass
            true_dist.fill_(self.smoothing / (self.cls - 1)) # the value is uniform distribution
            # the sum of other indexs == smoothing, and label index position == confidence,  smoothing + confidence = 1
            true_dist.scatter_(1, target.data.unsqueeze(1), self.confidence) #  ( dim, index, value ) is the value of scatter
            # target shape : (bz,) -> target.unsqueeze(1) shape : bz, 1
            # the true_dist is the softed soomthing label, the shape is same : bz, n_classes
        return torch.mean(torch.sum(-true_dist * pred, dim=self.dim))  # this implament sames not good.
