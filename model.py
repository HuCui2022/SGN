...
class SGN(nn.Module):
    def __init__(self, num_classes, dataset, seg, args, bias = True):
        ...
        # 这里就是部位和时间编码。
        if args.train:
            self.spa = self.one_hot(bs, num_joint, self.seg)     # 空间onehot bz,t,25,25
            self.spa = self.spa.permute(0, 3, 2, 1).cuda()     # bz,25,25,t
            self.tem = self.one_hot(bs, self.seg, num_joint)     # 时间 onehot.
        ...
        # 如果想要共享某个部分的权重，则将他做成一个子module 然后每次使用，都是共享的权重。
        # 如果不需要共享权重，则将他们做成多个模块，每个模块的参数都是不同的，也就不共享了。
        ...
        self.compute_g1 = compute_g_spa(self.dim1 // 2, self.dim1, bias=bias)  #计算注意力机制
    ...
    def forward(self, input):
        ...
        g = self.compute_g1(input)  #计算图矩阵类似的东西。
